...
MAX_OPEN_BODY_BYTES = 4 * 1024 * 1024 * 1024
...
class BrowserHandler(BoundedHandler):
    ...
    def _render_section(self, section: ApprovalSection) -> str:
        ...
        return (
            f'<section class="card preview-card preview-{section.kind}"><div class="section-heading"><div>'
            f'<p class="eyebrow">{eyebrow}</p><h2>{html.escape(section.title)}</h2>{summary}'
    ...
    def render_page(
        ...
        *,
        ...
        state_copy = {
            ...
            "executing": "The approved action is executing now. This page updates automatically.",
        ...
        primary_fields = list(fields[:4])
        technical_fields = list(fields[4:]) + [
        ...
        summary = f'<dl class="summary-grid">{primary_details}</dl>' if primary_details else ""
        technical = (
        ...
        if state == "pending":
            ...
            decision = (
                '<section class="card decision-card"><div class="decision-text"><p class="eyebrow">Decision required</p>'
                '<h2>Authorize this exact action?</h2>'
                '<p class="decision-copy">One immutable approval. At most one external write attempt.</p></div>'
                '<div class="actions">'
